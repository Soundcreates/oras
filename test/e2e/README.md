...
The GitHub Actions e2e job is a separate Kubernetes-based check. Coverage uploaded
to Codecov is generated by the unit-test job, so the e2e job does not publish a
second coverage profile.
